...
export function formatSSEEvent(data) {
  ...
  const STATUS_MSG = {
    ...
    manager_decided: `Manager 决策: ${data.action === "write" ? "撰写" : data.action === "polish" ? "润色" : data.action === "polish_fulltext" ? "全文润色" : data.action === "review" ? "审校" : data.action}`,
    chapter_writing: `正在撰写第 ${data.chapter} 章`,
    chapter_written: `第 ${data.chapter} 章撰写完成`,
    chapter_polishing: `正在${data.mode === "fulltext" ? "全文润色" : "润色"}第 ${data.chapter} 章 (第 ${data.polish_round} 轮)`,
    chapter_polished: `第 ${data.chapter} 章${data.mode === "fulltext" ? "全文润色" : "润色"}完成`,
    ...
    cycle_stuck: data.message || data.reason || `MWR 循环卡住`,
